...
function Chat(props) {
	...
	const handlerSend = async () => {
		...
		if (!roomId) {
			...
			socket.emit('join-room', newRoomData._id)
	...
	const fetchData = async () => {
		const _id = localStorage.getItem('njs_asm3_roomId')
		const response = await ChatRoomsAPI.getMessageByRoomId(_id);
		setMessage(response.message);
	};
	const createNewRoom = async () => {
		const newRoomData = await ChatRoomsAPI.createNewRoom();
		setRoomId(newRoomData._id);
		console.log('I emit', 79)
		socket.emit('join-room', { roomId: newRoomData._id })
		localStorage.setItem('njs_asm3_roomId', newRoomData._id);
		fetchData()
	}
							...
							<div className='ps-container ps-theme-default ps-active-y fix_scoll'>
								{message &&
									message.map((value, index) =>
										value.userId == localStorage.getItem('id_user') ? (
											<div
